...
function maxNonAdjacentSum(nums, memo = {}) {
    ...
    let a = nums[0] + maxNonAdjacentSum(nums.slice(2), memo)
    let b = maxNonAdjacentSum(nums.slice(1), memo);
    memo[nums] = Math.max(a, b);
    return memo[nums];
}
...
function minChange(coins, amount, memo = {}) {
    ...
    for (let i = 0; i < coins.length; i++) {
        if (amount >= coins[i]) {
            coinsCount.push(1 + minChange(coins, amount - coins[i], memo));
        }
    }
    memo[amount] = Math.min(...coinsCount);
    return memo[amount];
}
