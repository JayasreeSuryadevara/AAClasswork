...
function reverseString(str) {
    ...
    const latestLetter = str.slice(0,1);
    return reverseString(str.slice(1)) + latestLetter;
}
...
function pathFinder(directories, targetFile) {
    for (let fileName in directories) {
        if (fileName === targetFile ) return "/" + targetFile;

        let subDir = directories[fileName];
        let path = pathFinder(directories[fileName], targetFile);
        if (path !== null) {
