...
class MinMaxStack {
    ...
    push(val) {
        ...
        if (this.minStack.length === 0 || this.minStack[this.minStack.length - 1].value > val) {
            this.minStack.push(newNode);
        }
        if (this.maxStack.length === 0 || this.maxStack[this.maxStack.length - 1].value < val) {
    ...
    pop() {
        ...
        if (temp.value === this.min().value) this.minStack.pop();
        if (temp.value === this.max().value) this.maxStack.pop();
        return temp;
    }
